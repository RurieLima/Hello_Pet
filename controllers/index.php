...
class userController{
    ...
    static function info(){
        $user = new User();
        if(!empty($_GET["m"]) && !empty($_GET["name"])){
            $name = $user->clean_string($_REQUEST["name"]);
            $petData = $user->get_pet_info($name);        
            require_once("views/info.php");
        }       
    }
    //show view contact Pet
    ...
    //save contact 
    static function save_contact(){       
        $user = new User();
        if(!empty($_POST["contact_save"])){
            if(!empty($_POST["contact_msg"])){
                $name = $_POST["contact_name"];
                $_SESSION["msg"] = "Message sent successfully";
                $_SESSION["msgClass"] = "has-text-success";
                header("location:http://localhost/hello_pet/index.php?m=info&name=" .$name);
            }else{
                $name = $_POST["contact_name"];
                $_SESSION["msg"] = "Please insert your mesage in textarea field";
                $_SESSION["msgClass"] = "has-text-danger";
                header('location:http://localhost/hello_pet/index.php?m=contact&name='. $name);
            }
        }
    }
    ...
    static function is_favorite(){
         ...
         if(!empty($_GET["m"]) && !empty($_GET["id"])){
            $id = $user->clean_string($_GET["id"]);
            //recuperar email desde id
            $dataFavorite = $user->get_user_by_id($id);
            $email = $dataFavorite["user_email"];
            //data user
    ...
    static function save_gallery(){       
                ...
                //saving file imgs
                    ...
                    if (isset($archivo) && $archivo != ""){
                        ...
                        $temp = $_FILES['fileNew']['tmp_name'];
                         if (!((strpos($type, "gif") || strpos($type, "jpeg") || strpos($type, "jpg") || strpos($type, "png")) && ($size < 2000000))){
                             $_SESSION["msg_gallery"] = "Only .gif, .jpg, .png files are allowed. and 200 kb maximum";
                             $_SESSION["msgClass"] = "has-text-danger";
                         }else{
                             if (move_uploaded_file($temp, 'views/img/'.$archivo)){
                                 chmod('views/images/'.$archivo, 0777);
                             }else{
                                 $_SESSION["msg_gallery"] = "Error uploading file";
                                 $_SESSION["msgClass"] = "has-text-danger";
                             }
                         }
                     }
                 ...
                 $_SESSION["msg_gallery"] = "Data save success";
                 $_SESSION['msgClass'] = "has-text-success"; 
                 header("location:http://localhost/hello_pet/index.php?m=profile");   

            }else{
                $_SESSION["msg_gallery"] = "Please insert file photo";
                $_SESSION['msgClass'] = "has-text-danger"; 
                header("location:http://localhost/hello_pet/index.php?m=profile");
            }              
        }
        if(!empty($_POST["gallery_delete"]) && $_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            $_SESSION["msg_gallery"] = "Photo deleted";
            $_SESSION['msgClass'] = "has-text-success"; 
            header("location:http://localhost/hello_pet/index.php?m=profile"); 
        }
    }
